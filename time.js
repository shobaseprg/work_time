function main() {
  try {
    ...
    if (process.argv.length < 8) {
      ...
      console.log('');
      console.log('引数の説明:');
      console.log('  <勤務開始時間>: 普段業務を開始する時間。例) "9:25" [備考:退勤目安時間の表示に使われます。計算自体には使われないため適当でもいいです。]');
      ...
      console.log('        半休は含めない。半休の日は + 4働いたと脳内で計算してください。今月以外は入れても入れなくても計算は変わらないので入力しなくてもいいです。');
      console.log('        例) "[9/1,10/30]"');
      console.log('  <実出勤日数>: TS記載の実出勤日数。 例) 2 [備考 休暇日数は含まれていない]');
      console.log('  <総労働時間>: TS記載の総労働時間。 例) "160:00" [備考 当月の承認済み休暇は8時間加算されている]');
      console.log('  出勤中はややこしいので退勤後に実行してください');
      console.log('');
      console.log('使用例:');
